...
class AddQuestion extends Component {
  ...
  componentDidMount() {
    if (sessionStorage.getItem("authenticated") == "true") {
      this.setState({
        questionFormData: {
          competitionid: Number(this.props.match.params.competitionid),
          question: "",
          numcorrect: 1
        }
      });
  ...
  handleQuestionSubmit(event) {
    ...
    fetch('http://localhost:8080/create/question/' + this.props.match.params.competitionid, {
      method: 'POST',
      ...
      .then(question => {
        ...
        this.setState({
          message: "Вопрос добавлен! Теперь вы можете добавить параметры."
        });
      });
  }
  ...
  handleQuestionChange(event) {
    this.setState({
      questionFormData: {
        competitionid: this.state.questionFormData.competitionid,
  ...
  render() {
    return (
      <div className="container">
        <p className="display-3 text-center add-competition-heading">Новый вопрос</p>
          ...
          <div className="form-group row">
            ...
            <div className="col-sm-2">
              {!this.state.addOptions &&
                <button type="submit" className="btn btn-success btn-block">Добавить вопрос</button>
              }
              {this.state.addOptions &&
                <button type="submit" className="btn btn-success btn-block" disabled>Добавить вопрос</button>
              }
            </div>
            <div className="col-sm-2"></div>
          </div>
          ...
          <form onSubmit={this.handleOptionSubmit}>
            ...
            <div className="form-group row">
              ...
              <div className="col-sm-2 option-submit">
                <button type="submit" className="btn btn-success btn-block">Добавить опцию</button>
              </div>
              <div className="col-sm-2"></div>
            </div>
          </form>
        ...
        <div className="row">
          ...
          {this.state.markedCorrect >= this.state.numCorrect &&
            <div className="col-sm-4">
              <button className="btn btn-success btn-block" onClick={this.handleNextQuestion}>Новый вопрос</button>
            </div>
          }
          {this.state.markedCorrect >= this.state.numCorrect &&
            <div className="col-sm-4"></div>
